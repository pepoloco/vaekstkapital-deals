      ...
      <div style={{
        ...
        <div style={{ padding: "20px 24px", borderBottom: "1px solid #e2e6ea", background: "#ffffff", borderRadius: "14px 14px 0 0" }}>
          ...
          <div style={{
            ...
            <span>Event date: <strong style={{ color: "#111827" }}>{fmtDate(cutoffDate)}</strong></span>
              ...
              {before.length > 0 && (
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, paddingBottom: 10, borderBottom: "2px solid #bfdbfe" }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#1d4ed8", letterSpacing: ".06em", textTransform: "uppercase" }}>Before Event</div>
              ...
              {after.length > 0 && (
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, paddingBottom: 10, borderBottom: "2px solid #6ee7b7" }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#065f46", letterSpacing: ".06em", textTransform: "uppercase" }}>After Event</div>
  ...
  if (contacts.length === 0) {
    return (
      <div style={{ padding: "32px", textAlign: "center", color: "var(--ink3)", fontSize: 13 }}>
        No contacts found for this event list in HubSpot
      </div>
    )
  }
...
function EventCard({ c, summary, summaryLoading, onClick }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap", marginBottom: summary || summaryLoading ? 12 : 0 }}>
        <div>
          <div style={{ fontSize: 9, color: "var(--ink3)", letterSpacing: ".07em", textTransform: "uppercase", marginBottom: 2 }}>Event date</div>
        ...
        <div style={{ marginLeft: "auto" }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: "#b45309" }}>{c.participantCount}</div>
        </div>
      </div>
...
export default function InvestorDinnersPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/investor-dinners/campaigns")
      .then(r => r.json())
      .then(d => {
      ...
      .catch(e => { setError("Failed to load events: " + e); setCampaignsLoading(false) })
  }, [status])
  ...
  if (status === "loading" || (status === "authenticated" && canAccess(session?.user?.email) && campaignsLoading)) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "var(--bg)", gap: 8 }}>
        <div style={{ color: "var(--ink2)", fontSize: 14 }}>Loading investor dinner events…</div>
        <div style={{ color: "var(--ink3)", fontSize: 11 }}>Fetching event lists and attendee counts from HubSpot</div>
      </div>
    )
  }
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "var(--bg)", gap: 16 }}>
        ...
        <div style={{ fontSize: 13, color: "var(--ink3)", textAlign: "center", maxWidth: 360 }}>
          The Investor Dinners report is only available to VaekstHoldings and VK Fund Distribution users.
        </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
          <div>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, color: "var(--ink1)", margin: "0 0 4px" }}>Investor Dinners · Event Report</h1>
            <p style={{ fontSize: 12, color: "var(--ink3)", margin: 0 }}>
              {selected ? "Participant deals before and after the event date" : "Select an event to view participant and investment data"}
            </p>
          </div>
        ...
        {/* Event grid — split by country */}
        {!selected && (
          campaigns.length === 0 ? (
            <div style={{ background: "var(--card)", border: "1px solid var(--bdr)", borderRadius: 10, padding: "40px", textAlign: "center" }}>
              <div style={{ color: "var(--ink2)", fontSize: 13, marginBottom: 6 }}>No investor dinner lists found in HubSpot</div>
              <div style={{ color: "var(--ink3)", fontSize: 11 }}>Searched for lists named "DK - Event - ... - Attended"</div>
            ...
            const colHeader = (label: string, flag: string, count: number): React.ReactNode => (
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14, paddingBottom: 12, borderBottom: "2px solid var(--bdr)" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "var(--ink3)" }}>{count} event{count !== 1 ? "s" : ""}</div>
                </div>
              </div>
            )
              ...
              <div style={colStyle}>
                ...
                {items.length === 0
                  ...
                  : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      {items.map(c => <EventCard key={c.id} c={c} summary={summaries[c.id]} summaryLoading={summaryLoading[c.id]} onClick={() => openCampaign(c)} />)}
                    </div>
                }
              </div>
            )
            ...
            return (
              <div style={{ display: "flex", gap: 28, alignItems: "flex-start" }}>
                {dk !== null && renderCol(dk, "Denmark", "🇩🇰", "No Denmark events found")}
                {showBoth && <div style={{ width: 1, background: "var(--bdr)", alignSelf: "stretch", flexShrink: 0 }} />}
                {se !== null && renderCol(se, "Sweden", "🇸🇪", "No Sweden events found")}
              </div>
            )
          })()
        )}
          ...
          <>
            <div style={{
              ...
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                <div>
                  <div style={{ fontSize: 9, color: "var(--ink3)", letterSpacing: ".07em", textTransform: "uppercase" }}>Event date (cutoff)</div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: "#b45309" }}>{selected.participantCount}</div>
                </div>
                {reportData && (() => {
                  ...
                  const reinvestBDeals  = reinvestors.reduce((s, c) => s + c.beforeCount, 0)
                  const reinvestADeals  = reinvestors.reduce((s, c) => s + c.afterCount, 0)
                  const avgBefore = reinvestBDeals > 0 ? reinvestBefore / reinvestBDeals : 0
                  const avgAfter  = reinvestADeals > 0 ? reinvestAmt    / reinvestADeals  : 0
                  const avgDiff   = reinvestADeals > 0 && avgBefore > 0 ? avgAfter - avgBefore : null
                  return (
                    <>
